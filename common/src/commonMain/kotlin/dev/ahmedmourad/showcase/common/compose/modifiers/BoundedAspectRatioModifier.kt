...
/**
 * Exact same as the official aspectRatio modifier except that it never exceeds the max constraints
 *
 *
 ...
 */
@Stable
fun Modifier.boundedAspectRatio(
    /*@FloatRange(from = 0.0, fromInclusive = false)*/
    ratio: Float,
    matchHeightConstraintsFirst: Boolean = false
) = this.then(
    BoundedAspectRatioElement(
        ratio,
        matchHeightConstraintsFirst,
        debugInspectorInfo {
            name = "boundedAspectRatio"
...
private class BoundedAspectRatioElement(
    val aspectRatio: Float,
    val matchHeightConstraintsFirst: Boolean,
    val inspectorInfo: InspectorInfo.() -> Unit
) : ModifierNodeElement<BoundedAspectRatioNode>() {
    init {
        require(aspectRatio > 0) { "aspectRatio $aspectRatio must be > 0" }
    }

    override fun create(): BoundedAspectRatioNode {
        return BoundedAspectRatioNode(
            aspectRatio,
            matchHeightConstraintsFirst
        )
    }

    override fun update(node: BoundedAspectRatioNode) {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val otherModifier = other as? BoundedAspectRatioElement ?: return false
...
private class BoundedAspectRatioNode(
